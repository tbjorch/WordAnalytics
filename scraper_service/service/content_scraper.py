...
def _assert_not_payed_content(soup: BeautifulSoup) -> None:
    sportbladet_paywall = soup.find(
        style="background-image:url(//wwwe.aftonbladet.se/ab-se/\
            hyperion/gfx/logo-sportbladet-plus-2019.svg)"
    )
    aftonbladet_paywall = soup.find(
        style="background-image:url(//wwwe.aftonbladet.se/ab-se/\
            hyperion/gfx/logo-aftonbladet-plus-2019.svg)"
    )
...
def get_news_content(url_dto: UrlDTO) -> None:
    ...
    try:
        ...
        with db_lock:
            ...
            rpc.post_article(article)
            url_dto.scraped_at = datetime.now()
            # url_repo.update_url(url_dto)
            rpc.set_url_flag_to_scraped(url_dto.id, url_dto.scraped_at)
        ...
        with db_lock:
            ...
            # url_repo.update_url(url_dto)
        logging.warning(
            f"Payed content article does not allow\
                complete scraping of {url_dto.url}"
        )
    except Exception as e:
